...
export interface QItem {
  id: string
  type: "likert" | "scale" | "text" | "radio" | "checkbox" | "boolean"
  label: string
  options?: string[]
  required?: boolean
  min?: number
...
function isAnswered(item: QItem, v: unknown): boolean {
  ...
  if (item.type === "boolean") return v === true
...
export function QuestionnaireForm({
  ...
  const missing = items.filter(i => i.required && !isAnswered(i, answers[i.id]))
    ...
    <div className="mx-auto w-full max-w-2xl">
      ...
      <div className="flex flex-col gap-6">
        {items.map(item => {
          ...
          const err = showErrors && item.required && !isAnswered(item, v)
          return (
            <div key={item.id} className={cn("rounded-lg border p-4", err && "border-destructive")}>
              ...
              {err && <p className="mt-2 text-xs text-destructive">This question is required.</p>}
            </div>
          )
        })}
      </div>
...
function QuestionInput({ item, value, onChange }: {
  ...
  if (item.type === "likert" || item.type === "scale") {
